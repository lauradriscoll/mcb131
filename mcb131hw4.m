...
test_acc(2,epoch,P==P_set) = nanmean(Y_test==Y_testhat);

end

subplot(1,2,1)
hold on
plot(find(P==P_set),nanmean(train_acc(1,:,P==P_set)),'ok','MarkerFaceColor','k')
plot(find(P==P_set)+.1,nanmean(train_acc(2,:,P==P_set)),'ob','MarkerFaceColor','b')
plot(find(P==P_set)*ones(1,10),train_acc(1,:,P==P_set),'ok')
plot(find(P==P_set)*ones(1,10)+.1,train_acc(2,:,P==P_set),'ob')
xlim([0 3])
ylim([.3 1])
plot([0 3],[.5 .5],':k')
...
subplot(1,2,2)
hold on
plot(find(P==P_set),nanmean(test_acc(1,:,P==P_set)),'ok','MarkerFaceColor','k')
plot(find(P==P_set)+.1,nanmean(test_acc(2,:,P==P_set)),'ob','MarkerFaceColor','b')
plot(find(P==P_set)*ones(1,10),test_acc(1,:,P==P_set),'ok')
plot(find(P==P_set)*ones(1,10)+.1,test_acc(2,:,P==P_set),'ob')
xlim([0 3])
ylim([.3 1])
plot([0 3],[.5 .5],':k')
set(gca,'xtick',[1 2],'xticklabel',P_set)
...
legend('linear kernel','our kernel','location','southeast')
axis square
end
